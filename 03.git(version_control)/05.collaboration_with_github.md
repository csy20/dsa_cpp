...
2. **What is a Pull Request and why use it instead of direct merge?**
3. **How do you contribute to someone else's repository?**
4. **What does "forking" a repository mean?**
5. **When should you use `git push --force`?**
...
1. **Fetch vs Pull**:
   - `fetch`: Downloads changes but doesn't modify your files (safe)
   - `pull`: Downloads AND merges automatically (fetch + merge)

2. **Pull Request**:
   - Proposes changes from one branch to another
   - Enables code review before merging
   - Allows discussion and iteration
   - Creates audit trail
...
4. **Forking**:
   - Creates your personal copy of someone's repository
   - You have full control over your fork
   - Can propose changes back via Pull Request
   - Different from cloning (fork is on GitHub, clone is local)

5. **Force Push**:
   - **ONLY** on your own feature branches
   - **NEVER** on shared/main branches
   - Use `--force-with-lease` (safer)
   - Last resort after rebasing or amending pushed commits

</details>

---

## ⚠️ Common Collaboration Mistakes

### ❌ Mistake #1: Force Pushing to Main
**Problem**: Destroys team's work, causes conflicts

```bash
# DANGER: Never do this!
git push --force origin main  # ❌ Destroys everyone's work
```

**Solution**: ONLY force push to your feature branches
```bash
git push --force-with-lease origin my-feature-branch  # ✅ Safer
```

### ❌ Mistake #2: Merging Your Own Pull Requests
**Problem**: No code review, bugs slip through

**Solution**: Always wait for teammate approval

### ❌ Mistake #3: Not Syncing Before Starting Work
**Problem**: Working on outdated code

```bash
# BAD: Start work without syncing
git checkout -b feature/new-thing  # ❌ Main is outdated

# GOOD: Sync first
git checkout main
git pull origin main
git checkout -b feature/new-thing  # ✅ Up to date
```
...
**Next Steps**: Learn [Intermediate Techniques](06.intermediate_technique.md) for undoing changes, stashing work, and advanced Git operations.