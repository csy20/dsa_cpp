...
## Reset vs Revert vs Checkout - Comparison

| Command | What It Does | Safety | Use When | Public Branch? |
|---------|--------------|--------|----------|----------------|
| `git reset --soft HEAD~1` | Move HEAD, keep staged | ✅ Safe | Redo commit message | ❌ No |
| `git reset HEAD~1` | Move HEAD, unstage | ✅ Safe | Unstage files | ❌ No |
| `git reset --hard HEAD~1` | Move HEAD, delete changes | ⚠️ DANGER | Discard everything | ❌ Never |
| `git revert HEAD` | Create undo commit | ✅ Safe | Undo public commits | ✅ Yes |
| `git checkout -- file` | Restore from HEAD | ⚠️ Careful | Discard unstaged changes | ✅ Yes |
| `git restore file` | Modern restore | ✅ Safe | Same as checkout | ✅ Yes |

### Decision Tree: How to Undo?
...
├─ NO (Local only)
│  │
│  ├─ Want to fix commit message?
│  │  └─→ git commit --amend -m "new message"
│  │
│  ├─ Want to unstage files?
│  │  └─→ git restore --staged <file>
│  │
│  ├─ Want to undo commit (keep changes)?
│  │  └─→ git reset --soft HEAD~1
│  │
│  └─ Want to undo commit (delete changes)?
...
└─ YES (Already pushed)
   ...
   └─ Working with team?
      └─→ git revert <commit-hash>
         (Safest option - creates new commit)
```
...
3. **Can you recover commits after `git reset --hard`?**
4. **What does `git stash` do?**
5. **What's the purpose of git tags?**
...
1. **Reset Soft vs Hard**:
   - `--soft`: Moves HEAD, keeps changes staged (safe)
   - `--hard`: Moves HEAD and DELETES changes (dangerous)
   - `--mixed` (default): Moves HEAD, unstages changes

2. **Revert vs Reset**:
   - Use `revert` when commit is already pushed (safe for teams)
   - Use `reset` when commit is local only (rewrites history)

3. **Recover After Hard Reset**:
   - YES, sometimes! Use `git reflog` to find lost commits
   - Only works if commit was created (not just modified files)
   - Limited time window (usually 30-90 days)

4. **Git Stash**:
   - Temporarily saves uncommitted changes
   - Clears working directory
   - Can apply changes later
   - Useful for switching branches quickly

5. **Git Tags**:
   - Mark specific commits (usually releases)
   - Provide human-readable names (v1.0.0)
   - Create stable reference points
   - Help with releases and rollbacks
...
### ❌ Mistake #1: Using `git reset --hard` Without Checking
**Problem**: Permanently loses uncommitted work

```bash
# DANGER: This deletes all your changes!
git reset --hard HEAD  # ❌ All uncommitted work is GONE

# BETTER: Check what you're discarding first
git status
git diff
# Then if you're sure:
git reset --hard HEAD
...
### ❌ Mistake #2: Forgetting to Pop Stash
**Problem**: Changes stuck in stash, confusion later

**Solution**:
```bash
# After stashing, remember to apply it back
git stash
# ... do something ...
git stash pop  # ✅ Don't forget this!

# Or list stashes to see what's saved
git stash list
```

### ❌ Mistake #3: Rebasing Public Branches
**Problem**: Causes conflicts for everyone

```bash
# DANGER: Never rebase main/shared branches
git checkout main
git rebase feature  # ❌ NEVER!

# SAFE: Only rebase local feature branches
git checkout feature
git rebase main  # ✅ OK if feature hasn't been pushed
```
...
| **Undo last commit (delete)** | `git reset --hard HEAD~1` ⚠️ |
| **Undo public commit** | `git revert HEAD` |
...
| **Save with message** | `git stash save "message"` |
...
| **Annotated tag** | `git tag -a v1.0.0 -m "Release v1.0"` |
| **List tags** | `git tag` |
...
✅ `stash` for temporary save  
✅ `reflog` can recover lost commits  
❌ Never `reset --hard` without backup  
...
**Congratulations!** You've completed the Git fundamentals series. For additional help, refer to the [Git Cheat Sheet](git-cheat-sheet.md) for quick command reference.